...
const Login = () => {
    ...
    return(
        <div className="bg-white pt-6 pb-10  rounded-3xl lg:mt-16 w-full shadow-lg flex-row lg:w-full">
            {isLoading 
            ?<Spinner/>
            :<><h1 className="text-[#00df9a] text-2xl italic uppercase font-bold w-full text-center">Login</h1>
            {messages.length > 0
            ? messages.map((message) => <ValidationMessage key={message} message={message}/>)
            : null
            }
            <form onSubmit={loginHandler}>
                <input type="text" name="email" className="py-2 w-2/3 ml-[17%] md:w-1/3 md:ml-[33.5%] mb-2 mt-6 border-2 border-green-300 hover:border-green-100 rounded-md" placeholder="Email:vladislavdorovski@abv.bg" />
                <input type="password" name="password" className="py-2 w-2/3 ml-[17%] md:w-1/3 md:ml-[33.5%] border-2 border-green-300 hover:border-green-100 rounded-md" placeholder="Password:" />
                <div className="w-2/3 ml-[17%] md:w-1/3 md:ml-[33.5%] flex justify-around mt-4">
                    ...
                    <Link to={'users/register'} className="text-[#00df9a] font-bold">Sign in</Link>
                </div>
